<template>
  <div class="m-records">
    <header class="m-records-header">
      ...
      <span class="m-records-title">验证记录</span>
    ...
    <div v-loading="loading" class="m-records-body">
      ...
      <div v-for="item in records" :key="item.id" class="m-record-card">
        <div class="m-record-head">
          ...
          <span class="m-record-date">{{ item.verification_date || '-' }}</span>
        </div>
        <div class="m-record-info">{{ item.verification_info }}</div>
        <div class="m-record-meta">
          <span>下次验证：{{ item.next_verification_date || '-' }}</span>
      ...
      <el-empty v-if="!loading && records.length === 0" description="暂无验证记录" />
    </div>

    <el-drawer v-model="formVisible" :title="form.id ? '编辑验证记录' : '新增验证记录'" direction="btt" size="88%">
      <el-form label-position="top">
        ...
        <el-form-item label="验证信息" required>
          <el-input v-model="form.verification_info" type="textarea" :rows="4" placeholder="请输入本次验证的详细信息" />
        </el-form-item>
        <el-form-item label="验证时间" required>
          <el-date-picker v-model="form.verification_date" type="date" placeholder="选择日期" style="width: 100%" />
        </el-form-item>
        <el-form-item label="下次验证时间" required>
          <el-date-picker v-model="form.next_verification_date" type="date" placeholder="选择日期" style="width: 100%" />
        </el-form-item>
...
const loadRecords = async () => {
  loading.value = true
  try {
    records.value = await getVerificationRecords({ instrument_id: props.instrument.id })
  } finally {
    loading.value = false
  }
}
...
const saveRecord = async () => {
  if (!form.verification_info || !form.verification_date || !form.next_verification_date) {
  ...
  try {
    ...
    if (form.id) {
      await updateVerificationRecord(form.id, data)
    } else {
      await createVerificationRecord(data)
    }

    ElMessage.success('验证记录已保存')
